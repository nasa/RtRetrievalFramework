...
FtsConfig = ConfigCommon:new()

------------------------------------------------------------
--- Base class for FTS L1B creators
------------------------------------------------------------

FtsConfig.level1b_fts = CreatorL1b:new()

function FtsConfig.level1b_fts:sub_object_key()
   return {}
end

function FtsConfig.level1b_fts:create_parent_object()
    error("Abstract base class, use one of the derived FTS L1B Creator classes")
end

function FtsConfig.level1b_fts:register_output(ro)
   CreatorL1b.register_output(self, ro) 
...
FtsConfig.level1b_fts_tccon = FtsConfig.level1b_fts:new()

function FtsConfig.level1b_fts_tccon:create_parent_object()
   ...
   self.config.input_file_description = self.config.input_file_description .. 
	 "Runlog input file:   " .. self.config.runlog_file .. "\n"

   return l1b_fts
end

------------------------------------------------------------
--- Create a level 1b file using spectra and runlog information 
--- from a HDF file, with fields named like those for FTS
--- output in the L2 single sounding output
------------------------------------------------------------

FtsConfig.level1b_fts_hdf = FtsConfig.level1b_fts:new()

function FtsConfig.level1b_fts_hdf:create_parent_object()
   local hfile = HdfFile(self.config.spectrum_1_file)
   l1b_fts = Level1bFts(hfile, self.config.common.hdf_band_name)
...
function FtsConfig:l1b_number_pixel()
...
FtsConfig.fts_instrument = CompositeCreator:new()

function FtsConfig.fts_instrument:sub_object_key()
   return {"dispersion", "instrument_correction"}
end

function FtsConfig.fts_instrument:create_parent_object(sub_object)
   local ils = VectorIls()
   for i =1,self.config.l1b:number_spectrometer() do
...
function FtsConfig.fts_instrument:add_to_statevector(sv)
...
function FtsConfig:surface_pressure_from_runlog()
   -- Convert mb to Pa
...
------------------------------------------------------------
--- Create Chapman BOA RT using corrected apparent surface
--- SZA
------------------------------------------------------------

FtsConfig.chapman_boa_rt_sza_calculate = Creator:new()

function FtsConfig.chapman_boa_rt_sza_calculate:create()
   if(self.config.diagnostic) then
