...
SUBROUTINE LIDORT_WRITE_STATUS &
        ( THREAD, ERRORFILE, ERRORUNIT, OPENFILEFLAG, LIDORT_Status )
...
!  Copy inputs to local variables
      ...
      NCHECKMESSAGES                  = &
        LIDORT_Status%TS_NCHECKMESSAGES
      CHECKMESSAGES(0:NCHECKMESSAGES) = &
        LIDORT_Status%TS_CHECKMESSAGES(0:NCHECKMESSAGES)
      ACTIONS(0:NCHECKMESSAGES)       = &
        LIDORT_Status%TS_ACTIONS(0:NCHECKMESSAGES)
...
!  Check status and write messareus

      IF ( STATUS_INPUTCHECK .EQ. LIDORT_WARNING ) THEN
        WRITE(W,*)' WARNING: Suspicious input from LIDORT input check, Thread number '//C2
        ...
        DO N = 1, NCHECKMESSAGES
          NF = LEN_STRING(CHECKMESSAGES(N))
          NA = LEN_STRING(ACTIONS(N))
          write(W,'(A,I3,A,A)')'Message # ',N,' : ',CHECKMESSAGES(N)(1:NF)
          write(W,'(A,I3,A,A)')'Action  # ',N,' : ',ACTIONS(N)(1:NA)
        ENDDO
      ELSE IF ( STATUS_INPUTCHECK .EQ. LIDORT_SERIOUS ) THEN
        WRITE(W,*)' FATAL:   Wrong input from LIDORT input check, Thread number '//C2
...
!  Model calculation status
      ...
      IF ( STATUS_INPUTCHECK .NE. LIDORT_SERIOUS ) THEN
       IF ( STATUS_CALCULATION .NE. LIDORT_SUCCESS ) THEN
        WRITE(W,*)' FATAL: LIDORT execution failure, Thread number '//C2
