...
!  LIDORT module dependencies
   ...
   USE lidort_lc_miscsetups        ! Need all these
   ...
   USE lidort_lc_wfatmos,     only : UPUSER_COLUMNWF,  DNUSER_COLUMNWF,  MIFLUX_COLUMNWF, &
                                     GET_LC_BOASOURCE, GET_LC_TOASOURCE, LIDORT_LC_CONVERGE
...
SUBROUTINE LIDORT_LCS_master ( THREAD, &
        LIDORT_FixIn,    & ! INPUTS
...
!  Column weighting functions
      ...
      REAL(fpk) :: COLUMNWF ( MAX_ATMOSWFS,   MAX_USER_LEVELS, &
                              MAX_GEOMETRIES, MAX_DIRECTIONS, MAXTHREADS )

!  Mean-value weighting functions

      REAL(fpk) :: MINT_COLUMNWF ( MAX_ATMOSWFS, MAX_USER_LEVELS, &
                                   MAXBEAMS,     MAX_DIRECTIONS, MAXTHREADS )

!  Flux weighting functions

      REAL(fpk) :: FLUX_COLUMNWF ( MAX_ATMOSWFS, MAX_USER_LEVELS, &
                                   MAXBEAMS,     MAX_DIRECTIONS, MAXTHREADS )
...
!  Fourier-component surface weighting functions at user angles
      ...
      REAL(fpk)       :: SURFACEWF_F ( MAX_SURFACEWFS,   MAX_USER_LEVELS, &
                                       MAX_USER_STREAMS, MAXBEAMS, MAX_DIRECTIONS )
...
!  Linearized optical inputs
      ...
      L_DELTAU_VERT_INPUT(1:N_TOTALCOLUMN_WFS,1:NLAYERS,T) = &
        LIDORT_LinFixIn%Optical%TS_L_DELTAU_VERT_INPUT&
          (1:N_TOTALCOLUMN_WFS,1:NLAYERS,T)
      L_OMEGA_TOTAL_INPUT(1:N_TOTALCOLUMN_WFS,1:NLAYERS,T) = &
        LIDORT_LinFixIn%Optical%TS_L_OMEGA_TOTAL_INPUT&
          (1:N_TOTALCOLUMN_WFS,1:NLAYERS,T)
      L_PHASMOMS_TOTAL_INPUT(1:N_TOTALCOLUMN_WFS,0:NMOMENTS_INPUT,1:NLAYERS,T) = &
        LIDORT_LinFixIn%Optical%TS_L_PHASMOMS_TOTAL_INPUT&
          (1:N_TOTALCOLUMN_WFS,0:NMOMENTS_INPUT,1:NLAYERS,T)
...
!  New 15 March 2012
      ...
      COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,:,:,T) = ZERO
      MINT_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:,T)  = ZERO
      FLUX_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:,T)  = ZERO
...
!  Exception handling
      ...
      if ( fail ) then
        ...
        TRACE_3 = ' ** LIDORT_LCS_MASTER, THREAD # '//wthread
...
!  Could be done outside the thread loop
      ...
      IF ( DO_SOLAR_SOURCES ) THEN
        IF ( DO_CHAPMAN_FUNCTION ) THEN
          ...
          IF (FAIL) THEN
            TRACE_2 = 'Direct call in LIDORT_LCS_MASTER'
            TRACE_3 = ' ** LIDORT_LCS_MASTER, THREAD # '//wthread
            STATUS_CALCULATION = LIDORT_SERIOUS
            LIDORT_Out%Status%TS_STATUS_CALCULATION = STATUS_CALCULATION
            LIDORT_Out%Status%TS_MESSAGE = MESSAGE
...
!  column linearization
        ...
        ( DO_UPWELLING, DO_DNWELLING, DO_SSCORR_TRUNCATION,               & ! Input
          ...
          N_TOTALCOLUMN_WFS, THREAD, SS_FLUX_MULTIPLIER,                  & ! Input
          N_GEOMETRIES, UMOFF, STERM_LAYERMASK_UP, STERM_LAYERMASK_DN,    & ! Input
          PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,   & ! Input
...
!              LIDORT_LCS_FOURIER_MASTER
          ...
          CALL LIDORT_LCS_FOURIER_MASTER ( THREAD, FOURIER_COMPONENT, &
            ...
            INITIAL_TRANS, AVERAGE_SECANT, LC_INITIAL_TRANS, LC_AVERAGE_SECANT,                                            & !Input
...
!  Column Jacobians
      ...
      LIDORT_LinOut%Atmos%TS_COLUMNWF&
        (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_GEOMETRIES,:,T) = &
          COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:N_GEOMETRIES,:,T)
      LIDORT_LinOut%Atmos%TS_MINT_COLUMNWF&
        (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:,T)       = &
          MINT_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:,T)
      LIDORT_LinOut%Atmos%TS_FLUX_COLUMNWF&
        (1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:,T)       = &
          FLUX_COLUMNWF(1:N_TOTALCOLUMN_WFS,1:N_USER_LEVELS,1:NBEAMS,:,T)
...
SUBROUTINE LIDORT_LCS_FOURIER_MASTER ( THREAD, FOURIER_COMPONENT, &
        DO_UPWELLING, DO_DNWELLING, DO_SOLAR_SOURCES, DO_BRDF_SURFACE, DO_THERMAL_EMISSION, DO_SURFACE_EMISSION,       & !Input
        DO_THERMAL_TRANSONLY, DO_PLANE_PARALLEL, DO_USER_STREAMS, DO_SOLUTION_SAVING, DO_BVP_TELESCOPING,              & !Input
        DO_ADDITIONAL_MVOUT, DO_MVOUT_ONLY, DO_REFRACTIVE_GEOMETRY, DO_MSMODE_LIDORT, DO_MULTIBEAM, DO_MSMODE_THERMAL, & !Input
        NLAYERS, NSTREAMS, N_USER_STREAMS, N_USER_LEVELS, NBEAMS, NMOMENTS, NSTREAMS_2, NTOTAL,                        & !Input
...
!  Mean intensity weighting functions
      ...
      REAL(fpk), intent(inout) :: MINT_COLUMNWF &
        ( MAX_ATMOSWFS, MAX_USER_LEVELS, &
          MAXBEAMS,     MAX_DIRECTIONS, MAXTHREADS )

!  Flux weighting functions

      REAL(fpk), intent(inout) :: FLUX_COLUMNWF &
        ( MAX_ATMOSWFS, MAX_USER_LEVELS, &
          MAXBEAMS,     MAX_DIRECTIONS, MAXTHREADS )
...
!  Linearizations of Holding arrays for Multiplier coefficients
      ...
      REAL(fpk)  :: LC_GAMMA_M (MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
      REAL(fpk)  :: LC_GAMMA_P (MAXSTREAMS,MAXLAYERS,MAX_ATMOSWFS)
...
!                     done instead inLC_BVP_SOLUTION_MASTER
...
!  Solve the linearized BVP (No telescoped solution)
            ...
            ( DO_INCLUDE_SURFACE, DO_BRDF_SURFACE, DO_LOCALBEAM(IBEAM),   & ! Input
              ...
              N_TOTALCOLUMN_WFS, FOURIER_COMPONENT, IBEAM, QUAD_STRMWTS,  & ! Input
...
!  TOA source term linearized
            ...
            CALL GET_LC_TOASOURCE &
              ( N_USER_STREAMS, LC_TOA_SOURCE, N_TOTALCOLUMN_WFS )
...
!  ----------------------------------------------------------
          ...
          IF ( DO_INCLUDE_MVOUTPUT ) THEN
            ...
            ( DO_UPWELLING, DO_DNWELLING, DO_LOCALBEAM(IBEAM),   & ! Input
              THREAD, IBEAM, N_TOTALCOLUMN_WFS,                  & ! Input
...
!  telescoped BVP linearization
          ...
          ( DO_PLANE_PARALLEL, NSTREAMS, NSTREAMS_2,              & ! Input
            ...
            N_TOTALCOLUMN_WFS, FOURIER_COMPONENT, IBEAM,          & ! Input 
...
!  TOA source term linearized
              ...
              CALL GET_LC_TOASOURCE  ( N_USER_STREAMS, LC_TOA_SOURCE, N_TOTALCOLUMN_WFS )
