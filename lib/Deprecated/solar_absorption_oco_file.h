#ifndef SOLAR_ABSORPTION_OCO_FILE_H
#define SOLAR_ABSORPTION_OCO_FILE_H
...
public:
  ...
  virtual ~SolarAbsorptionOcoFile() {}
...
//-----------------------------------------------------------------------
  ...
  int number_line() const {return freq.rows();}
...
private:
  std::string hdf_file_name, hdf_group;
  double fraction_solar_diameter_;
				// Fraction of solar diameter viewed.
  // These names match the names in the Fortran code. This is
  // frequency, optical thickness, folding width and doppler width.
  // The units of freq, w_wid and d_wid are held fixed at inv_cm,
  // since this is what the underlying Fortran requires. We
  // ensure these are the units of the data when we read it in the
  // constructor (converting if needed).
  blitz::Array<double, 1> freq, stren, w_wid, d_wid;
};
}
#endif
