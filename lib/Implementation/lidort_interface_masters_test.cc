...
{
  Brdf_Sup_Masters brdf_master(1);
  ...
  Array<double, 3> bs_exactdb_brdfunc_expect;
  expected_data >> bs_exactdb_brdfunc_expect;

  BOOST_CHECK_MATRIX_CLOSE(brdf_outputs.bs_exactdb_brdfunc()
                           (r_u_streams, r_u_realazms, r_beams), bs_exactdb_brdfunc_expect);
...
BOOST_AUTO_TEST_CASE(lidort_ls_brdf_master)
{
  ...
  Brdf_Linsup_Masters brdf_master(1);
  ...
  Array<double, 4> bs_ls_exactdb_brdfunc_expect;
  expected_data >> bs_ls_exactdb_brdfunc_expect;

  BOOST_CHECK_MATRIX_CLOSE(brdf_outputs.bs_ls_exactdb_brdfunc()
                           (r_coef, r_u_streams, r_u_realazms, r_beams), bs_ls_exactdb_brdfunc_expect);
...
BOOST_AUTO_TEST_CASE(lidort_lps_master)
{
  ...
  // Same as lidrot_brdf_master except for weighting function parts
  Brdf_Linsup_Masters brdf_master(thread_idx+1);
  ...
  Lidort_Lps_Masters lps_master(thread_idx+1);
  ...
  // by 1
  Array<double, 1> lambertian_albedo(lid_pars.maxthreads);
  Array<double, 2> deltau_vert_input(lid_pars.maxlayers, lid_pars.maxthreads);
  Array<double, 2> omega_total_input(lid_pars.maxlayers, lid_pars.maxthreads );
  Array<double, 3> phasmoms_total_input(lid_pars.maxmoments_input+1, lid_pars.maxlayers, lid_pars.maxthreads);

  Array<double, 3> l_deltau_vert_input(lid_pars.max_atmoswfs,lid_pars.maxlayers,lid_pars.maxthreads);
  Array<double, 3> l_omega_total_input(lid_pars.max_atmoswfs,lid_pars.maxlayers,lid_pars.maxthreads);
  Array<double, 4> l_phasmoms_total_input(lid_pars.max_atmoswfs,lid_pars.maxmoments_input+1,lid_pars.maxlayers,lid_pars.maxthreads);
  ...
  lambertian_albedo(thread_idx) = 0.05e0;

  // rayleigh layers
  int n6 = nlayers - 6;
  for (int lay_idx = 0; lay_idx < n6; lay_idx++) {
    deltau_vert_input(lay_idx,thread_idx) = molext(lay_idx);
    omega_total_input(lay_idx,thread_idx) = molomg(lay_idx);
    phasmoms_total_input(0,lay_idx,thread_idx) = raymoms(0,lay_idx);
    phasmoms_total_input(1,lay_idx,thread_idx) = raymoms(1,lay_idx);
    phasmoms_total_input(2,lay_idx,thread_idx) = raymoms(2,lay_idx);
    double ratio1 = 1.0e0 - molomg(lay_idx);
    l_deltau_vert_input(0,lay_idx,thread_idx) =   ratio1;
    l_omega_total_input(0,lay_idx,thread_idx) = - ratio1;
  }
  ...
  for (int lay_idx = n6; lay_idx < nlayers; lay_idx++) {
    ...
    deltau_vert_input(lay_idx,thread_idx) = totext;
    omega_total_input(lay_idx,thread_idx) = omega;
    double ratio1 = molabs / totext;
    l_deltau_vert_input(0,lay_idx,thread_idx) =   ratio1;
    l_omega_total_input(0,lay_idx,thread_idx) = - ratio1;
    double ratio2 = aerext / totext;
    l_deltau_vert_input(1,lay_idx,thread_idx) = ratio2;
    l_omega_total_input(1,lay_idx,thread_idx) = ratio2 * ((waer/omega) - 1.0e0);
    phasmoms_total_input(0,lay_idx,thread_idx) = 1.0e0;
    for (int mom_idx = 1; mom_idx < 3; mom_idx++)
      phasmoms_total_input(mom_idx,lay_idx,thread_idx) = raywt * raymoms(mom_idx,lay_idx) + aerwt * aermoms(mom_idx);
    for (int mom_idx = 3; mom_idx < nmoments_input+1; mom_idx++)
      phasmoms_total_input(mom_idx,lay_idx,thread_idx) = aerwt * aermoms(mom_idx);
    for (int mom_idx = 1; mom_idx < nmoments_input+1; mom_idx++) {
      ratio2 = aersca / totsca;
      l_phasmoms_total_input(1,mom_idx,lay_idx,thread_idx) = ratio2 * 
        ( (aermoms(mom_idx)/phasmoms_total_input(mom_idx,lay_idx,thread_idx)) - 1.0e0 );
    }
  }
  ...
  BOOST_CHECK_MATRIX_CLOSE(lambertian_albedo, lambertian_albedo_exp);
  BOOST_CHECK_MATRIX_CLOSE(deltau_vert_input, deltau_vert_input_exp);
  BOOST_CHECK_MATRIX_CLOSE(omega_total_input, omega_total_input_exp);
  BOOST_CHECK_MATRIX_CLOSE(phasmoms_total_input, phasmoms_total_input_exp);

  BOOST_CHECK_MATRIX_CLOSE(l_deltau_vert_input, l_deltau_vert_input_exp);
  BOOST_CHECK_MATRIX_CLOSE(l_omega_total_input, l_omega_total_input_exp);
  BOOST_CHECK_MATRIX_CLOSE(l_phasmoms_total_input, l_phasmoms_total_input_exp);
  ...
  Array<double, 4> ts_intensity_calc( lid_output.ts_intensity() );
  BOOST_CHECK_MATRIX_CLOSE( ts_intensity_calc(rulevs,rgeoms,rdirs,0), 
                            ts_intensity_expt(rulevs,rgeoms,rdirs,0) );

  Array<double, 6> ts_profilewf_calc( lid_lpoutput.ts_profilewf() );
  BOOST_CHECK_MATRIX_CLOSE( ts_profilewf_calc(ratmwfs,rlayers,rulevs,rgeoms,rdirs,0), 
                            ts_profilewf_expt(ratmwfs,rlayers,rulevs,rgeoms,rdirs,0) );

  Array<double, 5> ts_surfacewf_calc( lid_lsoutput.ts_surfacewf() );
  BOOST_CHECK_MATRIX_CLOSE_TOL( ts_surfacewf_calc(rsurfwfs,rulevs,rgeoms,rdirs,0), 
                                ts_surfacewf_expt(rsurfwfs,rulevs,rgeoms,rdirs,0), 1e-6 );

  // These two are not used in production
  Array<double, 4> ts_mean_intensity_calc( lid_output.ts_mean_intensity() );
  BOOST_CHECK_MATRIX_CLOSE( ts_mean_intensity_calc(rulevs,rbeams,rdirs,0), 
                            ts_mean_intensity_expt(rulevs,rbeams,rdirs,0) );

  Array<double, 4> ts_flux_integral_calc( lid_output.ts_flux_integral() );
  BOOST_CHECK_MATRIX_CLOSE( ts_flux_integral_calc(rulevs,rbeams,rdirs,0), 
  ...
  Array<double, 6> ts_mint_profilewf_calc( lid_lpoutput.ts_mint_profilewf() );
  BOOST_CHECK_MATRIX_CLOSE( ts_mint_profilewf_calc(ratmwfs,rlayers,rulevs,rbeams,rdirs,0), 
                            ts_mint_profilewf_expt(ratmwfs,rlayers,rulevs,rbeams,rdirs,0) );

  Array<double, 6> ts_flux_profilewf_calc( lid_lpoutput.ts_flux_profilewf() );
  BOOST_CHECK_MATRIX_CLOSE( ts_flux_profilewf_calc(ratmwfs,rlayers,rulevs,rbeams,rdirs,0), 
