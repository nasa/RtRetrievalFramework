...
BOOST_FIXTURE_TEST_SUITE(zero_offset_waveform_output, GlobalFixture)
...
BOOST_AUTO_TEST_CASE(basic)
{
  ...
  boost::shared_ptr<ZeroOffsetWaveform> 
    zoff(new ZeroOffsetWaveform(1.0, true, *d, hf, 0, ils_tab->band_name()));

  ZeroOffsetWaveformOutput po(zoff, "o2");
  boost::shared_ptr<OutputHdf> out(new OutputHdf("zero_offset_waveform_output.h5", 20, 112, 5, 3));
  add_file_to_cleanup("zero_offset_waveform_output.h5");
