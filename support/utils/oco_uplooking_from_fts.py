...
from offline_ils import convolve_data
...
def create_sounding_id(obs_id, l1b_fts):
    dt = datetime.utcfromtimestamp(l1b_fts.time(0).unix_time)
    sounding_id = '%s%03d' % (dt.strftime('%Y%m%d%H%M%S'), obs_id)
    return sounding_id
...
def store_l1b_data(sounding_id, snd_idx, l1b_fts, output_file, l1b_file):
    ...
    for band_idx in range(len(BAND_NAMES)):
        ...
        conv_wl, conv_rad = convolve_data(l1b_file, high_res_wl, high_res_rad, band_idx, in_wavenumber=True)
