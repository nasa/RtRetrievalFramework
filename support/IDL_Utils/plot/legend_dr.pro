...
;	  28 Aug 92, FKK
;       changed name from legend.pro tp legend_dr.pro. Scaled symbol size to 0.8
;         of character size. 02 Dec 03, DMR
;-----------------------------------------------------------------------------------
pro legend_dr, help=help,items,linestyle=linestyle,psym=psym,vectorfont=vectorfont $
               ...
               ,fill=fill,usersym=usersym,corners=corners,out_of_range=out_of_range $
   ...
   for i = 0,n-1 do begin
       ...
       if vectorfont(i) ne '' then begin
           ...
           if symline and (linestyle(i) ge 0) then plots,xp,yp,color=colors(i) $
             ,/normal,linestyle=linestyle(i),psym=psym(i),symsize=0.8*charsize
       endelse
       ...
       if (n_elements(out_of_range) gt 0) then begin
           if (out_of_range[i] ne 0) then begin
               xyouts,x,y,'!U*!N',width=width,/norm,color=getRedIndex(),$
                      size=charsize
               x = x + width  
           endif
       endif
       ...
       if (n_elements(fill_value) gt 0) then begin
           if (fill_value[i] ne 0) then begin
               xyouts,x,y,'!U*!N',width=width,/norm,color=getBlackIndex(),$
                      size=charsize
               x = x + width  
           endif
       endif
   ...
   if box then plots,[x,maxx,maxx,x,x],y + [0,0,ywidth,ywidth,0],/norm, color = 1
   return
end
