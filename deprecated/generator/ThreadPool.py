# Based on:
# http://code.activestate.com/recipes/196618/
import random
import sys
import threading
import time
...
class MultiThread:
    ...
    def _tailRecurse( self ):
        for args in self._argsIterator:
            # Make sure we can expand any passed lists as arguments
            if not hasattr(args, '__iter__') or type(args) is types.DictType:
                args = (args,)
...
def recite_n_times_table( n, f ):
    for i in range( 1, 13 ):
        print "%d %d * %d = %d" % (f, n, i, n * i)
        #time.sleep( 0.5 + random.random() )

if __name__=="__main__":
   mt = MultiThread( recite_n_times_table, range( 1, 13 ), {'f':20} )
   mt.start()
   mt.join()
