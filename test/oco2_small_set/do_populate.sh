...
source ${srcdir}/setup_env.sh && cd oco2_sounding_${val}_test && \
    create_config.py -t oco \
    ...
    if [ -e "$config_file" ]; then
        populate.py -b `pwd`/../${cmd} oco_oco2_sounding_${val}_test.config --l2_config=${config_file} && exit 0
    else
        populate.py -b `pwd`/../${cmd} oco_oco2_sounding_${val}_test.config && exit 0
    fi
exit 1
